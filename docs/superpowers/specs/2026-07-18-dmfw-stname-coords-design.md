...
**时长**（实测校准：5 县并发 2 + 800ms ≈ 0.8 req/s；50 县并发 3 + 500ms，抖动率 ~52%）：

- 并发 2 + 800ms 间隔：**~5-6 小时**（5 县实测外推）
- 并发 3 + 500ms 间隔：**~3.5-4 小时**（50 县实测外推，抖动重试占 ~52%）
- WAF：50 县并发 3 零封禁，可用；更高并发待试

> **修正历程**：v0.1 误标「县级 ~5-8 小时、村级数天」（未算清 `size=100` 批量化）；v0.2 误估 ~1.5-2.5h（未算礼貌间隔 + 翻页 + 抖动重试）。5 县实测并发2 0.8 req/s（~5-6h）；50 县实测并发3 抖动率 ~52%（稳定化全恢复、0 失败），全量 ~3.5-4h。与百科抓取（~2.7h）独立，断点续爬可分段完成。
