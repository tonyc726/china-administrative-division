...
- **管线判定**：按文件名/author 启发式（含 `xzqh`→xzqh，含 `dmfw`/`dmfw-crawler`→dmfw，否则 community）。`source_pipeline` 未入 patch schema（避免改两个 producer），故**原料文件名须可辨识来源**（如 `xzqh-2026.json`、`110000-dmfw-2026.json`）。
