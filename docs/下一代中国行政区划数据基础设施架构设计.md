...
```
graph TD
    Root[Monorepo Root] --> Core[@cn-division/core]
    Root --> CLI[@cn-division/cli]
    Root --> Crawler[@cn-division/crawler]
    Root --> Protocol[@cn-division/data-protocol]

    subgraph "External Data Lake (NPM Registry)"
        Data2023[@cn-division/source-2023]
        Data2025[@cn-division/source-2025]
    end
...
| 包名 | 职责描述 | 关键依赖 |
|------|----------|----------|
| `@cn-division/core` | **核心逻辑**：纯计算层。包含 TS 类型定义、Luhn 校验算法、级联推导逻辑。无副作用，可在 Browser/Node/Edge 运行 | Zero Dependency |
| `@cn-division/cli` | **编排控制**：项目的大脑。负责数据下载（Hydration）、SQLite 数据库维护、CSV 流式导出、补丁应用 | Bin Alias: `cn-div`<br>`better-sqlite3`, `cacache`, `tar-stream`, `got` |
| `@cn-division/data-protocol` | **协议定义**：定义 Patch JSON Schema、数据库 Schema、置信度枚举。被 CLI 和 Crawler 共享 | `zod` 或 `ajv` |
| `@cn-division/crawler` | **情报采集** (Dev Only)：包含针对民政部 PDF、地图 API 的特定爬虫 | `puppeteer`, `cheerio`, `pdf-parse` |
...
- **存储位置**：`~/.cn-division/cache.db`（用户本地）或 CI 环境临时构建
...
| 项目 | 说明 |
|------|------|
| **包名规范** | `@cn-division/source-<YEAR>`（如 `@cn-division/source-2025`） |
...
```bash
# 使用 pnpm dlx (推荐)
pnpm dlx @cn-division/cli hydrate --year 2025

# 或使用 npx
npx @cn-division/cli hydrate --year 2025
```
...
```bash
# 1. 安装 CLI 为开发依赖
pnpm add -D @cn-division/cli

# 2. 执行命令 (使用 bin alias 'cn-div')
pnpm exec cn-div hydrate --year 2025
```
...
1. **Schema Check**：符合 `@cn-division/data-protocol` 定义
...
pnpm exec cn-div export --format legacy-json --year 2023 --out ./dist
```
