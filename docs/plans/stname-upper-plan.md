...
- 更新 `memory/dmfw-stname-coords.md`：记录 upper 采集完成、数据口径、已知缺失
  - 更正（2026-09-25）：该路径实指 Claude 项目记忆（仓库无 `memory/` 目录），文件在
    `~/.claude/projects/-Users-zhujiangfeng-Playground-china-administrative-division/memory/dmfw-stname-coords.md`
- 更新本计划文件状态（勾选验收项）
- 验收：记忆文件与本计划状态一致
...
- [x] 阶段 3：`InfoPanel` 在省/市/县级显示坐标与高德链接
  - 实测（2026-09-25，vite preview + headless Chrome CDP 走查）：河北省 114.5244°E → 保定市 115.4582°E → 竞秀区 115.4524°E 逐级显示坐标 + 高德链接；点击乡级（先锋街道）后坐标区块按预期隐藏；console 零错误
- [x] 阶段 4：乡级仍无坐标，降级隐藏；记忆文件更新（2026-09-25，路径更正见阶段 4 节）
