...
`cndiv` 只负责把数据注水进 `cache.db`，查询推荐用 [`@cndiv/reader`](../reader)——薄封装 better-sqlite3、自动屏蔽复合主键与市辖区占位层两个坑：

```ts
const cn = openCache();
cn.findByCode(code, year);
cn.getChildren(parentCode, year, { skipPlaceholder: true });
```

也可自带 [`better-sqlite3`](https://github.com/WiseLibs/better-sqlite3) + 原生 SQL 直查 `divisions` 表（reader 即此封装）。
