...
- **复合主键 `(code, year)`**：每个查询都强制 `year`——同一码跨年并存，漏 year 会命中多行。
- **占位中间层**：NBS 在直辖市/省直管处插入 `level=2` 的占位节点（`PLACEHOLDER_NAMES`：`市辖区` / `县` / `省直辖县级行政区划` / `省直辖县级行政单位`），dmfw 扁平结构里没有。如北京（`110000`）的直接子级是「市辖区」（`110100`），`getChildren(..., { skipPlaceholder: true })` 按 `level=2 + 名称` 识别并穿透一层到真实区县。
