...
/**
 * NBS 在直辖市/省直管/自治区直管处插入的 level=2 占位中间层名称（dmfw 扁平结构中不存在）。
 *
 * **本项目唯一真相源**——差分侧（crawler/normalize）与查询侧（reader）必须共用这一份。
 * 曾经两边各存一份并漂移：`自治区直辖县级行政区划`（新疆 659000）两边都漏了，于是被当成
 * 真政区，在 dmfw 差分里产出假 remove。占位层判错的代价是往库里写一条假撤销，故清单只增不删，
 * 且改动必须回到这里。
 *
 * 校验：NBS 2023 全量里 level=2 的占位层共 8 个 —— 市辖区 ×4、省直辖县级行政区划 ×3、
 * 自治区直辖县级行政区划 ×1。
 */
