...
export function extractIntents(text: string): ChangeIntent[] {
  ...
  for (const m of text.matchAll(cheSheRe)) {
    ...
    push(
      { kind: 'establish', name: m[2], evidence: m[0] },
      `establish:${m[2]}`
    );
  }
  ...
  for (const m of rest.matchAll(
    ...
    push(
      { kind: 'rename', from: m[1], to: m[2], evidence: m[0] },
      `rename:${m[1]}->${m[2]}`
    );
  }
  // 3) 纯撤销
  ...
  for (const m of rest.matchAll(new RegExp(`设立(${NAME})`, 'g'))) {
    push(
      { kind: 'establish', name: m[1], evidence: m[0] },
      `establish:${m[1]}`
    );
  }
  // 5) 划归/划入：将X划归/划入Y(管辖)
  for (const m of rest.matchAll(
    ...
    push(
      { kind: 'transfer', target: m[1], newParent: m[2], evidence: m[0] },
      `transfer:${m[1]}->${m[2]}`
    );
  }
