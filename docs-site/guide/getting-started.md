...
> **关于村级计数 620573 vs 620572**：`620573` 是**原始 NBS 直采成品**的村级计数。而 `cndiv hydrate` 注水的**分发基线**会丢弃 NBS 的自指占位行（直筒子市、金门县等 `parent === code` 的补位记录），村级为 **620,572**——首页与图表所示即此。两者相差 1 属预期，非数据错误。
