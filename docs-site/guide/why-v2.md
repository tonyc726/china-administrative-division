...
这正是 v2 用复合主键 `(code, year)` 逐年版本化、而非只存一版全量的根因：只有把「同一个码在不同年份的不同含义」都留存，才能回答「2015 年的 310115 是什么」这类时点查询。

## 与旧库的差异化

相比只提供「某一版全量」的社区库，v2 提供了三个核心能力：

- **历年版本化**：复合主键 `(code, year)`，同一区划码可跨年份并存，支持时点查询。
- **后统计局时代仍可更新**：官方停更后，仍能通过 dmfw 事件流 + 社区 Patch 持续维护，不依赖单一活源。
- **完整性可校验**：数据包 `manifest.json` 带 SHA-512，快照 Release 带逐文件 SHA-256，分发链路可审计。
